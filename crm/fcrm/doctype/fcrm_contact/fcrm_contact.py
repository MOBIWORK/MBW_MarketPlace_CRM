...
class FCRMContact(Document):
	...
	def autoname(self):
		...
		if frappe.db.exists("FCRM Contact", self.name):
			self.name = append_number_if_name_exists("FCRM Contact", self.name)
...
def get_default_contact(doctype, name):
	...
	out = frappe.db.sql(
		"""select parent,
			IFNULL((select is_primary_contact from `tabFCRM Contact` c where c.name = dl.parent), 0)
...
def update_contact(doc, method):
	...
	contact_name = frappe.db.get_value("FCRM Contact", {"email_id": doc.name})
	if contact_name:
		contact = frappe.get_doc("FCRM Contact", contact_name)
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	...
	return frappe.db.sql(
		f"""select
			`tabFCRM Contact`.name, `tabFCRM Contact`.full_name, `tabFCRM Contact`.company_name
		from
			`tabFCRM Contact`, `tabDynamic Link`
		where
			`tabDynamic Link`.parent = `tabFCRM Contact`.name and
			`tabDynamic Link`.parenttype = 'FCRM Contact' and
			`tabDynamic Link`.link_doctype = %(link_doctype)s and
			`tabDynamic Link`.link_name = %(link_name)s and
			`tabFCRM Contact`.`{searchfield}` like %(txt)s
			{get_match_cond(doctype)}
		order by
			if(locate(%(_txt)s, `tabFCRM Contact`.full_name), locate(%(_txt)s, `tabFCRM Contact`.company_name), 99999),
			`tabFCRM Contact`.idx desc, `tabFCRM Contact`.full_name
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = frappe.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "FCRM Contact"})
	if not link_fields:
		return []
	...
	return frappe.get_list("FCRM Contact", fields=fields, filters={"name": ("in", contact_names)})
...
def get_contact_display_list(doctype: str, name: str) -> list[dict]:
	...
	if not frappe.has_permission("FCRM Contact", "read"):
		return []
	...
	contact_list = frappe.get_list(
		...
		filters=[
			...
			["Dynamic Link", "parenttype", "=", "FCRM Contact"],
		],
		fields=["*"],
		order_by="is_primary_contact DESC, `tabFCRM Contact`.creation ASC",
	)

	for contact in contact_list:
		contact["email_ids"] = frappe.get_all(
			"Contact Email",
			filters={"parenttype": "FCRM Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)
