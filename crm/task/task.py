...
def send_reminders():
	...
	doc_test.label = str(pending_reminders) + str(upper_threshold) + str(lower_threshold)
	doc_test.save()
...
def send_notify_for_reminder(name):
		if name is None:
			return
		frappe.db.set_value('Reminder', name, 'notified', 1)
		reminder_info = frappe.get_doc('Reminder', name)
